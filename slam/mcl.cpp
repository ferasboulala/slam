...
namespace slam
{
MCL::MCL(const Lidar& lidar, int n_particles, const std::array<double, 4> alphas) : lidar(lidar), m_alphas(alphas)
{
...
void MCL::predict(const Odometry& odom)
{
    for (Particle& particle : this->particles)
    {
        particle.pose = sample_motion_model_odometry(odom, particle.pose, this->m_alphas);
    }
}
...
void MCL::update_inner(const std::vector<double>& scans, int start, int n)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        {
            double w = measurement_model_beam(scans[j], this->lidar.stddev, particle, this->lidar.max_dist);
            weight += std::log(w);
...
{
    assert(scans.size() == this->lidar.n_rays && "Number of reported scans do not match lidar number of rays");

    std::vector<std::thread> threads(std::thread::hardware_concurrency());
    const int number_particles_per_thread = this->particles.size() / threads.size();
    for (unsigned i = 0; i < threads.size(); ++i)
    {
        const int start = i * number_particles_per_thread;
        int n = number_particles_per_thread;
        if (i == threads.size() - 1) n += this->particles.size() % threads.size();
...
{
    std::vector<Particle> new_particles = probabilistic_fitness_selection(this->particles.size());
