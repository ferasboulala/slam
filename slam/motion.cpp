                             ...
                             std::array<double, 4> alphas)
{
    const double rotation_1 =
      odom.rotation_1 - sample_normal_distribution(
                          std::sqrt(alphas[0] * std::pow(odom.rotation_1, 2) +
                                    alphas[1] * std::pow(odom.translation, 2)));
    const double translation =
      odom.translation - sample_normal_distribution(
                           std::sqrt(alphas[2] * std::pow(odom.translation, 2) +
                                     alphas[3] * std::pow(odom.rotation_2, 2)));
    const double rotation_2 =
      odom.rotation_2 - sample_normal_distribution(
                          std::sqrt(alphas[0] * std::pow(odom.rotation_2, 2) +
                                    alphas[1] * std::pow(odom.translation, 2)));
