                                  ...
                                  std::array<double, 4> alphas)
{
    const double rotation_1 =
        odom.rotation_1 -
        sample_normal_distribution(std::sqrt(alphas[0] * std::pow(odom.rotation_1, 2) +
                                  ...
                                  std::array<double, 6> alphas)
{
    ...
    const double v = vel.v + sample_normal_distribution(std::sqrt(alphas[0] * std::pow(vel.v, 2) +
                                                                  alphas[1] * std::pow(vel.w, 2)));
    const double w = vel.w + sample_normal_distribution(std::sqrt(alphas[2] * std::pow(vel.w, 2) +
                                                                  alphas[3] * std::pow(vel.v, 2)));
    const double gamma = sample_normal_distribution(
        std::sqrt(alphas[4] * std::pow(vel.v, 2) + alphas[5] * std::pow(vel.w, 2)));

    const double x =
        pose.x -
        v / (w == 0 ? 1 : w) * (std::sin(pose.theta) + std::sin(pose.theta + (w + EPSILON) * dt));
    const double y =
        pose.y +
        v / (w == 0 ? 1 : w) * (std::cos(pose.theta) - std::cos(pose.theta + (w + EPSILON) * dt));
